<x-filament::page>
    <x-filament::tabs label="Content">
        <x-filament::tabs.item :active="$activeTap === 1" wire:click="$set('activeTap', 1)">
            {{ __('settings.profile-settings.top_heading') }}
        </x-filament::tabs.item>

        <x-filament::tabs.item :active="$activeTap === 2" wire:click="$set('activeTap', 2)">
            {{ __('settings.invoice-settings.top_heading') }}
        </x-filament::tabs.item>
    ...
    @if ($activeTap == 1)
        {{-- tap 1 --}}
        <x-filament::card header="{{ __('settings.profile-settings.top_heading') }}">
    ...
    @if ($activeTap == 2)
        {{-- tap 2 --}}
        <x-filament::card header="{{ __('settings.invoice-settings.top_heading') }}">
